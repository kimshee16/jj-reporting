    ...
    if ($type === 'all' || $type === 'campaigns') {
        $campaigns_sql = "
            SELECT 
                ...
                CONCAT('campaign_details.php?id=', c.campaign_id) as url,
                c.created_at,
                'campaign' as icon,
                c.status,
                c.total_spend,
    ...
    if ($type === 'all' || $type === 'ads') {
        $ads_sql = "
            SELECT 
                ...
                CONCAT('ad_details.php?id=', a.id) as url,
                a.created_time,
                'ad' as icon,
                a.status,
    ...
    if ($type === 'all' || $type === 'adsets') {
        $adsets_sql = "
            SELECT 
                ...
                CONCAT('adset_details.php?id=', a.id) as url,
                a.created_time,
                'adset' as icon,
                a.status,
                a.daily_budget,
    ...
    if ($type === 'all' || $type === 'accounts') {
        $accounts_sql = "
            SELECT 
                ...
                CONCAT('account_details.php?id=', faa.act_id) as url,
